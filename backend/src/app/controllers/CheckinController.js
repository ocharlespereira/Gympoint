...
class CheckinController {
  ...
  async store(req, res) {
    ...
    // Query of dates between seventhDayAgo and today(actual date)
    ...
    if (checkins >= 5) {
      return res.status(400).json({
        error:
          'Student already has 5 checkins in the last 7 days. Access denied!',
      });
    }

    const { id, student_id } = await Checkin.create({ student_id: student.id });
